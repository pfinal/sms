 ...
 *
 * 发手机验证码
 * $bool = $app['sms']->sendCode('18888888888', '888888');
 *
 * $app['sms'] = function () {
 *       $sms = new \SMS\CCPRestSMS();
 *       $sms->accountSid = '8a48b111d49643014fef4909c34302';
 *       $sms->accountToken = 'ca65c30c054444804242332df594';
 *       $sms->appId = '8a48b55aaaa0f40150513aca826ad2';
 *       $sms->templateId = '41116';
 *       return $sms;
 * };
 *
 ...
 * @since   1.0
 */
class CCPRestSMS implements SmsVerifyInterface, SmsTemplateInterface
{
    //主帐号
    public $accountSid;
    ...
    //请求地址，格式如下，不需要写https://
    //public $serverIP = 'sandboxapp.cloopen.com';    //开发
    public $serverIP = 'app.cloopen.com';         //生产
    ...
    public function sendCode($phone, $code, &$error = null)
    {
        //发送验证码消息模板 必须两个参数
        $datas = ["$code", 1];
        return $this->sendTemplateSMS($phone, $this->templateId, $datas, $error);
    }

    public function templateSMS($phone, $templateId = null, $params = array(), &$error = null)
    {
        return $this->sendTemplateSMS($phone, $templateId, $params, $error);
    }
     ...
     * @return bool
     */
    protected function sendTemplateSMS($to, $tempId, $datas, &$error = null)
    {
        // 初始化REST SDK
        ...
        if ($result == NULL) {
            Log::error("yuntongxun sendTemplateSMS result error!");
            $error = "yuntongxun sendTemplateSMS result error!";
            return false;
        }

        if ($result->statusCode != 0) {
            Log::error("yuntongxun error code :" . $result->statusCode . ", error msg :" . $result->statusMsg);
            $error = json_encode((array)$result);
            return false;
        } else {
            return true;
        }
...
class REST
{
    ...
    function showlog($log)
    {
        if ($this->enabeLog) {
            ...
            Log::error("yuntongxun enabeLog " . $log);
        }
    }
